...
Если локально нужен только фронт, а данные должны браться с production/VPS,
задайте в `.env`:

```dotenv
CIVIUM_API_URL=https://civium.online
```

После этого достаточно запускать `pnpm dev`; `pnpm api` локально не нужен.
Все чтение, ручные правки и импорты из локального интерфейса будут уходить в
API на VPS.
...
- Каждая новая Excel-выгрузка добавляет данные в общую базу: строки заново
  парсятся и классифицируются, совпавшие записи обновляются с сохранением ручных
  аннотаций, новые добавляются, а отсутствующие в новой выгрузке старые записи
  остаются в Civium.
